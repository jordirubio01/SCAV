...
# Funció Auxiliar per Encoding Ladder
def encode_variant(filename: str, width: int, height: int, bitrate: int, codec: str):
    input_path = f"/data/{filename}"
    output_filename = f"variant_{width}x{height}_{bitrate}k_{codec}_{filename}"
    output_path = f"/data/{output_filename}"

    if not os.path.exists(input_path):
        return {"error": f"El fitxer {filename} no existeix"}

    # ffmpeg -i input -vf scale=w:h -c:v h264 -b:v bitrate output
    command = [
        "ffmpeg", "-y",
        "-i", input_path,
...
def convert_codecs(filename: str):
    """
    Converteix un vídeo d'entrada a VP8, VP9, H.265 i AV1.
    """
    input_path = f"/data/{filename}"
    name, ext = os.path.splitext(filename)
    ...
    # Diccionari de còdecs i sortides
    codecs = {
        "vp8":  f"/data/{name}_vp8.webm",
        "vp9":  f"/data/{name}_vp9.webm",
        "h265": f"/data/{name}_h265.mp4",
        "av1":  f"/data/{name}_av1.mkv"
    }

    results = {}
    for codec, output_path in codecs.items():
        if codec == "vp8":
            command = ["ffmpeg", "-y", "-i", input_path, "-c:v", "libvpx", "-b:v", "1M", output_path]
        elif codec == "vp9":
            command = ["ffmpeg", "-y", "-i", input_path, "-c:v", "libvpx-vp9", "-b:v", "1M", output_path]
        elif codec == "h265":
        ...
        try:
            subprocess.run(command, check=True)
            results[codec] = {"status": "success", "output_file": os.path.basename(output_path)}
        except subprocess.CalledProcessError as e:
            results[codec] = {"status": "error", "detail": str(e)}
...
# Encoding Ladder
@app.post("/video/encoding-ladder/{filename}")
def encoding_ladder(filename: str):
    """
    Genera un encoding ladder amb vàries resolucions/bitrates cridant els endpoints interns
    """
    variants = [
        {"width": 1920, "height": 1080, "bitrate": 5000, "codec": "h264"},
        {"width": 1280, "height": 720,  "bitrate": 3000, "codec": "h264"},
        {"width": 854,  "height": 480,  "bitrate": 1500, "codec": "h264"},
        {"width": 426,  "height": 240,  "bitrate": 800,  "codec": "h264"},
    ]

    results = []
    for v in variants:
        res = encode_variant(filename, v["width"], v["height"], v["bitrate"], v["codec"])
        results.append({"variant": f"{v['width']}x{v['height']}@{v['bitrate']}k", "result": res})

    return {"status": "encoding_ladder_done", "results": results}