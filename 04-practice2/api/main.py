...
# Convertir Vídeo a Còdecs (VP8, VP9, H265, AV1) (utilitza l'altre Docker amb FFMPEG)
@app.post("/video/convert-codecs/{filename}")
def convert_codecs(filename: str):
    """
    Endpoint API que crida al servei FFMPEG per convertir un vídeo a VP8, VP9, H265 i AV1.
    """
    try:
        response = requests.post(f"http://ffmpeg:9000/video/convert-codecs/{filename}")
        if response.status_code != 200:
            raise HTTPException(status_code=response.status_code, detail=response.text)
        return response.json()
    except Exception as e:
...
# Encoding Ladder (utilitza l'altre Docker amb FFMPEG)
@app.post("/video/encoding-ladder/{filename}")
def encoding_ladder(filename: str):
    """
    Endpoint API que delega en el servei FFMPEG per generar un encoding ladder.
    """
    try:
        response = requests.post(f"http://ffmpeg:9000/video/encoding-ladder/{filename}")
        if response.status_code != 200:
            raise HTTPException(status_code=response.status_code, detail=response.text)
        return response.json()
    except Exception as e:
